...
function useColumnData(columnName: ColumnName, maxItems = 100) {
    ...
    useEffect(() => {
        ...
        Promise.all([indexPromise, colPromise]).then(([indices]) => {
            ...
            const sortedIndices = indices.sort((a, b) => column.data[b] - column.data[a]);
            setIndices(sortedIndices.slice(0, maxItems));
            setColumnData(newColumnData.sort((a, b) => b - a).slice(0, maxItems));
        });
        return () => {
            cancel = true;
        };
...
function useMarkdownText(md: string) {
    const [renderTextFn, setRenderTextFn] = useState<(md: string) => string>(null);
    const [html, setHtml] = useState<string>('');
    useEffect(() => {
        import('../../utilities/MarkdownText').then(({default: renderTextFn}) => {
...
export const HighlightedFeatureComponent = observer(() => {
    ...
    const {columnData, indices, column} = useColumnData(chart.config.param[0], 8);
    const highlightedIndex = useHighlightedIndex();
    const highlightInRange = indices && highlightedIndex >= 0 && indices.includes(highlightedIndex);
    ...
    return (
        <div style={{fontSize: '0.8rem'}}>
            ...
            <br />
            {highlightInRange ? `Highlighted value (one of the top values): ${highlightValue}` : `Highlighted value ${highlightValue} not in top ${columnData?.length} values.`}
            {columnData && [...columnData].map((d, i) => 
            <div 
            style={{cursor: 'pointer', border: `1px ${highlightedIndex === indices[i] ? 'solid' : 'dashed'} #ccc`, padding: '0.5em', margin: '0.5em', borderRadius: '0.5em'}}
            onClick={() => {chart.dataStore.dataHighlighted([indices[i]], chart)}} key={indices[i]}>
                {d}
            </div>
            )}
        </div>
