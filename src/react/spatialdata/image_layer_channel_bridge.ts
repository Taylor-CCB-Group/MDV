...
export function persistedChannelCount(
    ...
    return Math.max(channelIdCount, persistedMax, loaderMax, 1);
}
...
export function buildChannelDefaultsFromConfig({
    ...
    const domains = Array.from({ length: count }, (_, index) =>
        fallbackDomain(loaderDefaults?.contrastLimits?.[index] ?? contrastLimits[index]),
    );
    ...
    return {
        ...
        channelOptions: Array.from(
            { length: count },
            (_, index) => channelNames[index] ?? `Channel ${index + 1}`,
        ),
    };
}
...
export function mergeHydrateChannelsState(
    ...
    return {
        ...
        domains: defaults.ids.map(
            (id, index) => domainsById.get(id) ?? defaults.domains[index] ?? ([0, 255] as Range),
        ),
        raster: defaults.ids.map((id) => {
            const cached = rasterById.get(id);
            if (!cached) return EMPTY_RASTER;
            const data =
                cached.data instanceof Float32Array
                    ? cached.data
                    : Float32Array.from(cached.data);
            return { width: cached.width, height: cached.height, data };
        }),
    };
}
