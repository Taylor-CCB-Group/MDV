...
class MultiLineChart extends SVGChart {
    ...
    constructor(dataStore, div, config) {
        ...
        super(dataStore, div, config, {
            x: {},
            y: { type: config.stacked ? "band" : "linear" },
        });
    ...
    stackLines(st: boolean) {
        ...
        if (st) {
            ...
            this.setColorLegend();
            this.y_scale.domain(
                this.dataStore.getColumnValues(this.config.param[1]),
            );
        } else {
    ...
    drawChart() {
        ...
        for (let i = 0; i < this.data.length; i++) {
            const scale = c.stacked
                ? rowHeight / this.data[i].max
                : cdim.height / this.data[i].max;
            const arr = c.stacked
                ? this.data[i].map((x) => rowHeight * i + rowHeight - x * scale)
                : this.data[i].map((x) => cdim.height - x * scale);
        ...
        const ga = this.graph_area.selectAll(".chart-line").data(display_data);
        ga.join("path")
