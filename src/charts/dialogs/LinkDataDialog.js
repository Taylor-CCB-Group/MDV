...
class LinkDataDialog extends BaseDialog {
    ...
    init(content) {
        ...
        this.dataTypes =
            this.linkDataStore.columnGroups[this.link.columnGroup].subgroups;
        this.ids = new Set();
        createEl(
            "div",
            {
        ...
        this.list = createEl(
            "div",
            {
                styles: { flexGrow: 1, overflowX: "hidden" },
                overFlowY: "scroll",
            },
            lld,
        );
        ...
        createEl(
            "span",
            { classes: ["ciview-title-div"], text: "Data Type:" },
            d,
        );
    ...
    _addColorChartSection() {
        ...
        createEl(
            "div",
            { classes: ["ciview-title-div"], text: "Color Chart" },
            div,
        );

        createEl(
            "span",
            { text: "Chart:", styles: { marginRight: "4px" } },
            div,
        );
        ...
        const b = createEl(
            "button",
            { classes: ["ciview-button-sm"], text: "GO" },
            div,
        );
        b.addEventListener("click", () => {
        ...
        for (const cid in this.cm.charts) {
            ...
            if (cinfo.dataSource.name === this.link.dataSource) {
                ...
                if (ch.getColorOptions().colorby) {
                    createEl(
                        "option",
                        { value: cid, text: ch.config.title },
                        select,
                    );
                }
            }
    ...
    _addHeatMapSection() {
        ...
        createEl(
            "div",
            { classes: ["ciview-title-div"], text: "Add Heat Map" },
            div,
        );

        createEl(
            "span",
            { text: "Category:", styles: { marginRight: "4px" } },
            div,
        );
        const select = createEl(
            "select",
