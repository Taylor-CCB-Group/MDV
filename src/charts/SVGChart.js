...
class SVGChart extends BaseChart{
    constructor(dataStore,div,config,axisTypes){
        ...
        if (ya){
            ...
            this.y_axis_label=this.y_axis_svg.append("text")
    ...
    setAxisSize(axis,size){
        ...
        this.graph_area.attr("transform", `translate(${this.margins.left},${this.margins.top})`);
    ...
    updateAxis(){     
        ...
        this.ry_axis_svg.attr("transform",`translate(${dim.left+dim.width},${dim.top})`);
        this.tx_axis_svg.attr("transform",`translate(${dim.left},0)`);
        this.x_axis_svg.attr("transform", `translate(${dim.left},${this.height-this.margins.bottom})`);
        ...
        if (ax.x){
            if (this.x_scale){
                ...
                if (ax.x.rotate_labels){
                    ...
                    this.x_axis_svg.selectAll(".tick text").style("text-anchor", null)
                    .attr("dx", null)
                    .attr("dy", ".71em")
                    .attr("transform", null);
    
                }
                this.x_axis_svg.selectAll(".tick text").attr("font-size",ax.x.tickfont)
            } 
            this.x_axis_label
                .attr("x",dim.width/2)
        ...
        if (ax.y){
            if (this.y_scale){
                ...
                if (ax.y.rotate_labels){    
                    this.y_axis_svg.selectAll(".tick text").style("text-anchor", "end")
                    .attr("dy", "1em")
                    .attr("transform", "rotate(+45)");
                }
                else{
                    this.y_axis_svg.selectAll(".tick text").style("text-anchor", null)
                    .attr("dx", null)
                    .attr("dy", "0.32em")
                    .attr("transform", null);
                }
    ...
    drawXTree(nodeData,number,height,offset){
        ...
        for (let n of desc){
            if (!n.children){
                let i=n.parent;            
                while(i){
                    i.x=i.children[0].x+((i.children[1].x-i.children[0].x)/2);
                    i=i.parent;
                }
            }
        ...
        const links= this.tx_axis_svg.selectAll(".tree-link")
            .data(desc.slice(1))
            .join("path")
