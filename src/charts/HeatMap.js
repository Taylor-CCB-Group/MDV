...
class HeatMap extends SVGChart{
    constructor(dataStore,div,config){
		...
		super(dataStore,div,config,{x:{type:"band"},y:{type:"band"},tx:{custom:true,size:45}});
    ...
    drawChart(tTime=400){
        ...
        this.tx_axis_svg.selectAll(".heatmap-catsize")
        .data(cTotals,(d,i)=>{
          return  d[1];
        })
        .join(
          ...
          .call(exit => exit.transition(trans2)
            .attr("height", 0)
            .attr("y",45)
            .remove()
        ...
        if (this.rowClusterNodes){
            this.drawYTree(this.rowClusterNodes.nodes,this.config.param.length-1)
        }
        if (this.columnClusterNodes){
            this.drawXTree(this.columnClusterNodes.nodes,this.data.transpose.length,40,45)
        }
    }
