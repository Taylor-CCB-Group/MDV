...
onmessage = (e) => {
    ...
    const y =
        e.data[3][1] === "int32"
            ? new Int32Array(e.data[3][0])
            : new Float32Array(e.data[3][0]);
    const x =
        e.data[2][1] === "int32"
            ? new Int32Array(e.data[2][0])
            : new Float32Array(e.data[2][0]);
    const lFilter = new Uint8Array(e.data[0]);
    const gFilter = new Uint8Array(e.data[1]);
    const height = config.yscale[1][1];
    const yscale = scaleLinear()
        .domain(config.yscale[0])
        .range(config.yscale[1]);
    const xscale = scaleLinear()
        .domain(config.xscale[0])
        .range(config.xscale[1]);
    function calcDensity(category) {
        return contourDensity()
            ...
            .bandwidth(config.bandwidth)(gFilter);
    }
    const data = [];
