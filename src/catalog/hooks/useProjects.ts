...
const useProjects = () => {
  ...
  const createProject = useCallback(async () => {
    ...
    try {
      ...
      if (response.status === 500) {
        throw new Error(data.message || 'Error creating project. Please try again later.');
      }
  ...
  const deleteProject = useCallback(async (id: string) => {
    ...
    try {
      ...
      if (response.status === 403) {
        throw new Error('This project is read-only and cannot be modified');
      }
      
      if (response.status === 500) {
        throw new Error(data.message || 'Internal server error occurred while deleting project');
      }
  ...
  const renameProject = useCallback(async (id: string, newName: string) => {
    ...
    try {
      ...
      if (response.status === 500) {
        throw new Error(data.message || 'Internal server error occurred while renaming project');
      }
  ...
  const changeProjectType = useCallback(async (id: string, newType: "Editable" | "Read-Only") => {
    ...
    try {
      ...
      if (response.status === 400) {
        throw new Error('Invalid project type. Must be either "Editable" or "Read-Only"');
      }
      ...
      if (response.status === 500) {
        throw new Error(data.message || 'Internal server error occurred while updating project type');
      }
