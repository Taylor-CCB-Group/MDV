/**
 * Backend project from mock `.mdv.zip` (`create_test_project_zip` → `POST /import_project`)
 * with optional CSV fallback.
 *
 * Other flows: `syntheticAnndata.ts`, `syntheticSpatial.ts` (disk under `~/mdv` + `/rescan_projects`).
 */
...
async function setProjectEditable(request: APIRequestContext, projectId: string) {
    const response = await request.put(`/projects/${projectId}/access`, {
        form: {
            type: "editable",
        },
    });
...
export async function createTemporaryProject(
    ...
    try {
        ...
        await setProjectEditable(page.request, projectId);
