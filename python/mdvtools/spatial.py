def add_centroid_plots(project,datasource,split_by,ignore_values=[], scale=0.5):
    ...
    for sp_val, r_val in  zip(project.get_column(datasource,split_by), \
        ...
        for c,regions in enumerate(all_regions):
            ...
            for region in regions:
                ...
                if x>1000:
                    ...
                    max_y=0
                charts.append(chart)
            project.set_view(region_names[c],{
                "initialCharts":{
                    datasource:charts,
                }
            })
...
def create_interaction_matrix_view(project,interaction_datasource,exclude_groups=[],
                                   name="interaction view",
                                   cells_datasource="cells",
                                   interaction_metrics=["gr20"],square_size=20,
                                   images="pcf charts",
                                   selections=[
        ...
        if not i:
            raise ValueError(f"Datasource {interaction_datasource} does not have interaction data")
        ...
        project.set_view(name,{
                "initialCharts":{
                    cells_datasource:[],
                    interaction_datasource:charts
...
def create_roi_interaction_view(project,datasource,lchm=None,pcfs="pcf charts",
                                cells_datasource="cells",
                                extra_filters=[],
                                table_metrics=["gr20"],
