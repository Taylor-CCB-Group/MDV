...
def test_convert_scanpy_to_mdv(h5ad_file, test_project_dir):
    ...
    label = "test_"
    mdv_labeled = convert_scanpy_to_mdv(test_project_dir, adata, label=label, delete_existing=True)
...
def test_convert_scanpy_to_mdv_views(h5ad_file, test_project_dir):
    ...
    with pytest.raises(FileExistsError, match="Attempt to create datasource 'cells' failed because it already exists"):
    ...
    try:
        mdv_preserve = convert_scanpy_to_mdv(new_project_dir, adata, delete_existing=False)
