...
"""
nb - This doesn't actually test anything, but it does create a project with some data in it.
I had been thinking of `test_projects` as a place to put scripts that generate projects to be interacted with
and `tests` as a place for actual CI kind of tests...
but we're running `test_projects` when we run tests, and the previous output path was a problem for that.
"""
